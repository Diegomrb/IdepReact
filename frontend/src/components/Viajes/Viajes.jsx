import { Heading, Stack, FormControl, FormLabel, Input, Card, CardBody, CardFooter, CardHeader, Text } from '@chakra-ui/react';
...
export const Viajes = () => {
    ...
    let origen = pasaje.localidades.filter(item => item["_id"] == pasaje.origen)[0];
    let destino = pasaje.localidades.filter(item => item["_id"] == pasaje.destino)[0];
    return (
        <div>
            <Stack margin={"8px"} direction={["column"]} >
                ...
                <Stack direction={"row"}>
                    <FormControl>
                        ...
                        <Input variant='outline' placeholder='Origen' value={`${origen.nombre}, ${origen.departamento}`} disabled />
                    </FormControl>
                    <FormControl>
                        <FormLabel>Destino</FormLabel>
                        <Input variant='outline' placeholder='Destino' value={`${destino.nombre}, ${destino.departamento}`} disabled />
                    </FormControl>
                </Stack>
                <Stack direction={"row"}>
                    <FormControl>
                        <FormLabel>Fecha inicio</FormLabel>
                        <Input variant='outline' placeholder='Origen' value={pasaje.inicio} type="datetime-local" disabled />
                    </FormControl>
                    <FormControl>
                        <FormLabel>Fecha fin</FormLabel>
                        <Input variant='outline' placeholder='Destino' value={pasaje.fin} type="datetime-local" disabled />
                    </FormControl>
                </Stack>
            </Stack>

            <Stack direction={"column"}>
                {pasaje.viajes.map((viaje) => <PasajesCards key={viaje._id} info={viaje} />)}
            </Stack>
        </div>
    );
}
...
function PasajesCards({ info }) {
    ...
    let durTotalHoras = moment.duration(info.horaDeSalida).add(info.duracion, "minutes").asHours();
